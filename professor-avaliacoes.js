(() => {
  ...
  if (!lista || !resumo || !busca || !status) return;
  ...
  function resumoCurriculo(curriculo = {}, historico = false) {
    ...
    ].filter(Boolean).forEach((parte) => box.appendChild(parte));
  ...
  function totalEntrevista(form) {
    return ["comunicacao", "dominioCurriculo", "adequacaoVaga", "resolucaoProblema", "postura"]
      .reduce((total, nome) => total + (Number(form.elements[nome]?.value) || 0), 0);
  }
  ...
  function criarFormAvaliacao(processo, sessao) {
    ...
    topo.append(dataLabel, inputNumero("Nota do currículo (0 a 10)", "notaCurriculo", avaliacao.notaCurriculo, 10));
    form.appendChild(topo);

    form.appendChild(el("strong", "", "Entrevista — 5 critérios de 0 a 2 pontos"));
    ...
    form.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        console.error("Falha ao salvar avaliação:", erro);
  ...
  function criarCard(processo, sessao) {
    ...
    card.dataset.vagaId = processo.vagaId || "";
  ...
  async function buscarPorVagas(colecao, vagas) {
    const todos = [];
    for (const vaga of vagas) {
      try {
        const snap = await api.db.collection(colecao).where("vagaId", "==", vaga.id).get();
        snap.docs.forEach((doc) => todos.push({ id: doc.id, ...doc.data() }));
      } catch (erro) {
        console.warn(`Falha ao consultar ${colecao} da vaga ${vaga.id}.`, erro);
      }
    }
    return todos;
  }
  ...
  async function carregar(sessao) {
    const vagasSnap = await api.db.collection("vagas")
    ...
    const [submissoes, avaliacoes] = await Promise.all([
      buscarPorVagas("submissoes", vagas),
      buscarPorVagas("avaliacoes", vagas)
    ]);

    const submissoesValidas = submissoes.filter((item) => item.tipoDocumento !== "pdf_chunk");
    window.PROFESSOR_SUBMISSOES = submissoesValidas;

    const avaliacoesDoProfessor = avaliacoes.filter((item) => !item.professorUid || item.professorUid === sessao.usuario.uid);
    const avaliacoesPorId = new Map(avaliacoesDoProfessor.map((avaliacao) => [avaliacao.id, avaliacao]));
    const idsUsados = new Set();

    processos = submissoesValidas.map((dados) => {
      const avaliacao = avaliacoesPorId.get(dados.id)
        || avaliacoesDoProfessor.find((item) => item.submissaoId === dados.id)
        || null;
      if (avaliacao) idsUsados.add(avaliacao.id);
      return { ...dados, avaliacao };
    });

    avaliacoesDoProfessor.forEach((avaliacao) => {
  ...
  async function iniciar() {
    ...
    window.PROFESSOR_AVALIACOES_PRONTO = true;
    window.dispatchEvent(new CustomEvent("professor-avaliacoes-pronto"));
  }

  iniciar().catch((erro) => {
    console.error("Falha no painel de avaliações:", erro);
